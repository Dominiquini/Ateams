...
JobShop::JobShop() : Problem::Problem() {
	...
	if (Random::randomPercentage() <= 50) { // Tenta uma solucao gulosa
		...
		for (int i = 0; i < nmaq; i++) {
			shuffle(&aux_vet[0], &aux_vet[njob], randomEngine);
		...
		for (int i = 0; i < nmaq; i++) {
			...
			shuffle(&solution.scaling[i][0], &solution.scaling[i][njob], randomEngine);
		}
	}
...
inline Problem* JobShop::neighbor() {
	int maq = Random::randomNumber(0, nmaq), p1 = Random::randomNumber(0, njob), p2 = Random::randomNumber(0, njob);
	Problem *job = NULL;
...
inline vector<pair<Problem*, InfoTabu*>*>* JobShop::localSearch(float parcela) {
	...
	for (int i = 0; i < def; i++) {
		maq = Random::randomNumber(0, numMaqs), p1 = Random::randomNumber(0, numJobs), p2 = Random::randomNumber(0, numJobs);

		while (p2 == p1)
			p2 = Random::randomNumber(0, numJobs);
...
inline pair<Problem*, Problem*>* JobShop::crossOver(const Problem *parceiro, int partitionSize, int strength) {
	...
	shuffle(&maqs[0], &maqs[nmaq], randomEngine);
