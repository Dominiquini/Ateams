...
TravellingSalesman::TravellingSalesman() : Problem::Problem() {
	...
	if (random(0, 5) == 0) { // Tenta uma solucao gulosa
		int currentNode = random(0, nnodes);
		int nextNode = 0;
		int position = 0;
		double d = 0;
...
inline Problem* TravellingSalesman::neighbor() {
	int p1 = random(0, nnodes + 1), p2 = random(0, nnodes + 1);
	Problem *prob = NULL;

	while (p2 == p1)
		p2 = random(0, nnodes + 1);
...
inline vector<pair<Problem*, InfoTabu*>*>* TravellingSalesman::localSearch(float parcela) {
	...
	for (int i = 0; i < def; i++) {
		p1 = random(0, nnodes + 1), p2 = random(0, nnodes + 1);
